...
-- Step 11: Add unique constraint to prevent duplicate profiles per user
-- (Optional - uncomment if you want only one profile per user)
-- ALTER TABLE freelancer_profiles ADD CONSTRAINT unique_user_profile UNIQUE (user_id);

-- Step 12: Update existing profiles to ensure they have valid user references
-- This will create users for any orphaned profiles
UPDATE freelancer_profiles 
SET user_id = user_id 
WHERE user_id NOT IN (SELECT id FROM users);

-- Step 13: Create a view for easier profile management
...
COMMENT ON TABLE freelancer_profiles IS 'Freelancer profiles with automatic user creation support';
COMMENT ON FUNCTION ensure_user_exists(VARCHAR) IS 'Ensures a user exists before profile creation';
COMMENT ON FUNCTION trigger_ensure_user_exists() IS 'Trigger function to automatically create users for profiles';
COMMENT ON TRIGGER ensure_user_exists_trigger ON freelancer_profiles IS 'Automatically creates users before profile creation';

-- Step 15: Grant necessary permissions
GRANT SELECT, INSERT, UPDATE, DELETE ON freelancer_profiles TO postgres;
GRANT SELECT, INSERT, UPDATE, DELETE ON users TO postgres;
GRANT SELECT, INSERT, UPDATE, DELETE ON categories TO postgres;
GRANT SELECT ON freelancer_profiles_with_users TO postgres;

-- Step 16: Create a function to safely create freelancer profile
...
COMMENT ON FUNCTION create_freelancer_profile_safe IS 'Safely creates or updates a freelancer profile with automatic user creation';

-- Migration completed successfully
SELECT 'Migration 0004_fix_freelancer_profile_fk completed successfully' as status;
