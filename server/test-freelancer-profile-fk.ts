...
async function testFreelancerProfileFK() {
  ...
  try {
    ...
    try {
      ...
      if (error.code === '23503') {
        ...
        console.log('❌ Unexpected error:', error.message);
         ...
         // Test 6: Test duplicate profile creation (note: no unique constraint in DB, but business logic should prevent this)
     ...
     try {
       ...
       console.log('⚠️ Note: Database allows duplicate profiles (no unique constraint), but business logic should prevent this');
     } catch (error) {
       if (error.code === '23505') {
    ...
    try {
     ...
     console.log('- ⚠️ Duplicate profile prevention: Database allows duplicates, but business logic handles this');
